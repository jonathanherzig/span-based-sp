...
@SpanExtractor.register("start_end_point")
class EndpointSpanExtractor(SpanExtractor):
    ...
    def forward(
        ...
        if not self._use_exclusive_start_indices:
            ...
            start_embeddings = util.batched_index_select(sequence_tensor, span_starts)[:, :, :int(sequence_tensor.shape[2]/2)]
            end_embeddings = util.batched_index_select(sequence_tensor, span_ends)[:, :, int(sequence_tensor.shape[2]/2):]
